...
export const restHandlers = [
    rest.get(`${store.state.API_ENDPOINT}/api/organizations/associated_members/${store.state.selectedOrganization.identifier}`, (req, res, ctx) => {
        return res(ctx.status(200), ctx.json(associate_members));
    }),

    rest.get(`${store.state.API_ENDPOINT}/api/${store.state.selectedOrganization.identifier}/streams`, (req, res, ctx) => {
        return res(ctx.status(200), ctx.json(streams.stream_list));
    }),

    rest.get(`${store.state.API_ENDPOINT}/api/${store.state.selectedOrganization.identifier}/k8s_json/schema`, (req, res, ctx) => {
        return res(ctx.status(200), ctx.json(streams.stream_details));
    }),

    rest.get(`${store.state.API_ENDPOINT}/api/${store.state.selectedOrganization.identifier}/org_users`, (req, res, ctx) => {
