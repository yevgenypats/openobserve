...
#[tracing::instrument(name = "service:search:datafusion:ballista_exec:sql", skip_all)]
pub async fn sql(
    ...
    let df_ctx = SessionContext::with_config_rt(session_config.clone(), Arc::new(runtime_env));
    ...
    let schema = if schema.is_none() || session.data_type.eq(&file_list::SessionType::Cache) {
        config = config.infer_schema(&df_ctx.state()).await.unwrap();
    ...
    let ballista_config = BallistaConfig::builder().build().expect("");

    let ballista_ctx = BallistaContext::standalone(&ballista_config, 2)
        .await
        .unwrap();
    ...
    ballista_ctx.register_table("tbl", Arc::new(table))?;
    ...
    // query
    let mut df = match ballista_ctx.sql(&sql.origin_sql).await {
        Ok(df) => df,
        Err(e) => {
            log::error!(
    ...
    for (name, sql) in sql.aggs.iter() {
        ...
        let mut df = match ballista_ctx.sql(&sql.0).await {
            Ok(df) => df,
            Err(e) => {
                log::error!(
    ...
    // drop table
    //ballist_ctx.deregister_table("tbl")?;
    log::info!(
